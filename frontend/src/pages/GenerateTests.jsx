...
import { 
  ...
  Settings2, FolderGit2, MessageSquareQuote 
} from "lucide-react";
...
const LOADING_STATES = [
  ...
  "Finalizing JSON schema..."
];

export default function GenerateTests() {
  const navigate = useNavigate();
  const [url, setUrl] = useState("");
  const [customPrompt, setCustomPrompt] = useState(""); // NEW: State for user instructions
  ...
  const handleGenerate = async (e) => {
    ...
    try {
      await generateTests({
        ...
        customPrompt // Send the custom text to backend
      });
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-2xl">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl">
          <h2 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
            <Wand2 className="text-cyan-400" /> AI Architect
          </h2>
          <p className="text-slate-400 mb-8">Define your target and guide the AI's focus.</p>

          <form onSubmit={handleGenerate} className="space-y-6">
            {/* Project Select */}
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-300 flex items-center gap-2">
                <FolderGit2 size={16} /> Destination Project
              </label>
              <select 
                value={selectedProjectId}
                onChange={(e) => setSelectedProjectId(e.target.value)}
                className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3 text-white focus:ring-2 focus:ring-cyan-500 outline-none"
              >
            ...
            <div className="space-y-2">
              ...
              <textarea 
                placeholder="e.g. Focus on login validation, ignore the footer, test the search bar with empty values..."
                value={customPrompt} onChange={(e) => setCustomPrompt(e.target.value)}
                className="w-full bg-slate-800/50 border border-slate-700 rounded-xl p-3 text-slate-200 focus:ring-2 focus:ring-cyan-500 outline-none min-h-[100px] resize-none text-sm"
              />
            </div>
            ...
            <button
              ...
              className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${
                loading ? 'bg-slate-700 text-slate-400' : 'bg-cyan-500 hover:bg-cyan-600 text-slate-900 shadow-lg shadow-cyan-500/20'
              }`}
            >
              {loading ? (
                <><Loader2 className="animate-spin" /> {loadingText}</>
              ) : (
                <><CheckCircle2 size={20} /> Generate Professional Suite</>
              )}
            </button>

            {error && <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-sm flex items-center gap-2">
